...
print(f"Zakres wartości int16: {np.min(test_signal_int16)} do {np.max(test_signal_int16)}")
...
plt.plot(t[:500], filtered_int16_sim[:500], 'g--', label='Filtrowany (int16)', alpha=0.7)
...
for i, freq in enumerate(key_freqs):
    print(f"  {freq} Hz: {filt_float_amps[i]:.4f} (tłumienie: {20*np.log10(filt_float_amps[i]/orig_amps[i]):.1f} dB)")

print("\nAmplitudy po filtrowaniu (int16):")
for i, freq in enumerate(key_freqs):
    print(f"  {freq} Hz: {filt_int16_amps[i]:.4f} (tłumienie: {20*np.log10(filt_int16_amps[i]/orig_amps[i]):.1f} dB)")
