...
typedef enum {
    ...
    VC_CODEC_G711U   = 7,   /* G.711 µ-law, WAV fmt=7 */
    VC_CODEC_IMA_ADPCM = 0x11, /* IMA ADPCM, WAV fmt=0x11 (ADPCM Microsoft/IMA) */
    VC_CODEC_SPEEX_WB = 0xA1,  /* umowne ID na potrzeby VCMD (opcjonalnie) */
} vc_codec_id_t;
...
typedef struct VC_PACKED {
    ...
    int16_t* samples;           /* int16_t[len_samples] */
} vc_pcm_frame_t;
...
typedef struct VC_PACKED {
    ...
    /* Dla ADPCM: */
    uint16_t samples_per_block; /* np. 256 (ADPCM); 0 gdy nie dotyczy */
 ...
 */
static inline uint32_t vc_ima_block_bytes_mono(uint16_t samples_per_block) {
    uint32_t nibbles = (samples_per_block > 0) ? (uint32_t)(samples_per_block - 1) : 0u;
    uint32_t bytes_nibbles = (nibbles + 1u) / 2u; /* ceil */
    return 4u + bytes_nibbles;
}

/* Blok ADPCM jako chunk ogólny (payload = blok ADPCM dokładnie jak w WAV) */
typedef vc_stream_chunk_t vc_ima_adpcm_block_t;

/* ====== (Opcjonalnie) Ramka Speex WB ====== */
typedef vc_stream_chunk_t vc_speex_frame_t;
